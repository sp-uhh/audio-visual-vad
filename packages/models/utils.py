...
def method3(packed, lengths):
    ...
    (source: https://discuss.pytorch.org/t/get-each-sequences-last-item-from-packed-sequence/41118/7)

    Args:
        packed ([type]): [description]
...
def binary_cross_entropy(r, x, eps):
    ...
    return -torch.mean(x*torch.log(torch.sigmoid(r) + eps) + (1 - x)*torch.log(1 - torch.sigmoid(r) + eps)) # Average over (T,*)
    # return -torch.mean(x*torch.log(r + eps) + (1 - x)*torch.log(1 - r + eps)) # Average over (T,*)
